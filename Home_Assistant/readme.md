...
The frontend screenshot depicts the AquaPing operating with music playing nearby. An appreciable number of sample counts are being interpreted as noise causing the corresponding alert to trigger (On). Noise rejection is important for preventing false leak alarms.
...
There are many examples of sensors uploading data via I2C and displaying it on the dashboard. Going in the other direction, however, is not well documented. This capability is needed to send configuration parameters to the sensor.  In this demo, frontend data is passed through the ESPHome yaml to the firmware where it is sent as byte pairs using the Wires library. 
